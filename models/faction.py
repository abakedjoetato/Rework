...
class Faction:
    ...
    async def update(self, **kwargs) -> bool:
        ...
        if "owner_id" in kwargs:
            ...
            if isinstance(kwargs, dict) and kwargs["owner_id"] not in self.admin_ids:
                self.admin_ids.append(kwargs["owner_id"])
                update_fields["admin_ids"] = self.admin_ids
            if isinstance(kwargs, dict) and kwargs["owner_id"] not in self.member_ids:
    ...
    async def add_member(self, player_id: str) -> bool:
        ...
        # Check if player is not None is not None is in another faction
