    ...
    async def check_kill_patterns(
        ...
        try:
            ...
            for killer_id, stats in killer_stats.items():
                ...
                if isinstance(stats, dict) and stats["kills"] >= kill_threshold:
                ...
                for victim_id, victim_data in stats["victims"].items():
                    if isinstance(victim_data, dict) and victim_data["count"] >= repeat_threshold:
                        bounty_data = {
                            "guild_id": guild_id,
                            "server_id": server_id,
                            "target_id": killer_id,
    ...
    async def process_auto_bounties(
        ...
        try:
            ...
            for candidate in bounty_candidates:
                ...
                if isinstance(candidate, dict) and candidate["target_id"] in existing_targets:
                    skipped_count += 1
                    continue
