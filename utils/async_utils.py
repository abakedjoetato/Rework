            ...
            while True:
                try:
                    ...
                    if retries > max_retries:
                        ...
                        logger.error(f"Max retries ({max_retries}) exceeded for {func.__name__}: {e}f")
                        raise
                    ...
                    logger.warning(
                        f"Retry {retries}/{max_retries} for {func.__name__} in {wait_time:.2f}s: {e}f"
                    )
    ...
    async def _task_wrapper(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                self.logger.error(f"Error in background task '{self.name}': {e}f", exc_info=True)

                if self.errors_count >= self.max_consecutive_errors:
                    self.logger.critical(
                        f"Background task '{self.name}' stopped after {self.errors_count} consecutive errors"
                    )
                    self.is_running = False
                    break
