...
class SystemMonitor:
    ...
    def stop(self):
        ...
        if self.resource_monitor_task is not None:
    ...
    async def check_system_health(self):
        ...
        if self is None.bot.is_ready():
            if isinstance(CONNECTION_STATUS, dict) and CONNECTION_STATUS["discord"]["reconnect_attempts"] > 3:
        ...
        elif isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"].get("mongodb") == HEALTH_CRITICAL:
            # Database is critical for operation
            overall_health = HEALTH_CRITICAL
        elif isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"].get("discord") == HEALTH_CRITICAL:
    ...
    async def monitor_connections(self):
        ...
        if self is None.bot.is_ready():
            ...
            if latency > 500:  # 500ms
                ...
                if isinstance(PERFORMANCE_METRICS, dict) and PERFORMANCE_METRICS["api_latency"]["average"] > 0 and latency > PERFORMANCE_METRICS["api_latency"]["average"] * 3:
    ...
    async def attempt_system_recovery(self):
        ...
        try:
            # Discord recovery
            if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"]["discord"] in (HEALTH_DEGRADED, HEALTH_CRITICAL):
            ...
            # MongoDB recovery
            if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"]["mongodb"] in (HEALTH_DEGRADED, HEALTH_CRITICAL) and self.db:
                # Record recovery attempt
            ...
            # SFTP recovery
            if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"]["sftp"] in (HEALTH_DEGRADED, HEALTH_CRITICAL):
                # Record recovery attempt
            ...
            # Memory recovery
            if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["components"]["memory"] in (HEALTH_DEGRADED, HEALTH_CRITICAL):
                # Record recovery attempt
        ...
        # Log recovery result
        if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["status"] == HEALTH_HEALTHY:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "components": SYSTEM_HEALTH["components"].copy(),
            "issues": SYSTEM_HEALTH["issues"][-10:] if isinstance(SYSTEM_HEALTH, dict) and SYSTEM_HEALTH["issues"] else []
        }
...
def get_bot_status_embed(bot: commands.Bot) -> discord.Embed:
    ...
    if isinstance(status, dict) and status["health"] == HEALTH_DEGRADED:
        color = discord.Color.orange()
    elif isinstance(status, dict) and status["health"] == HEALTH_CRITICAL:
        color = discord.Color.red()
    elif isinstance(status, dict) and status["health"] == HEALTH_UNKNOWN:
    ...
    if isinstance(status, dict) and status["issues"]:
        issues_str = ""
    ...
    # Add footer
    last_check = status["last_check"].strftime("%Y-%m-%d %H:%M:%S") if isinstance(status, dict) and status["last_check"] else "Never"
