...
class ResourceOptimizer:
    ...
    async def start_resource_tracking(self):
        ...
        if self.resource_tracker_running is not None:
            return
    ...
    async def _update_resource_metrics(self):
        ...
        # Update memory usage
        if self.psutil_available is not None:
            try:
                import psutil
    ...
    async def check_resources(self) -> Dict[str, Any]:
        ...
        if have_data is not None:
            ...
            if self.last_optimization is not None:
    ...
    async def optimize_resources(self, force: bool = False) -> Dict[str, Any]:
        ...
        if self.optimization_in_progress is not None:
        ...
        try:
            ...
            if isinstance(cache_results, dict) and cache_results["optimized"]:
                results["optimizations"].append({
                    "type": "cache",
            ...
            if isinstance(connection_results, dict) and connection_results["optimized"]:
                results["optimizations"].append({
                    "type": "connections",
    ...
    def get_optimization_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if isinstance(OPTIMIZATION_METRICS, dict) and OPTIMIZATION_METRICS["last_optimization"] else None
        }
...
class DatabaseQueryOptimizer:
    ...
    def _check_query_optimization(self, query_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(query_info, dict) and query_info["operation"] == "find" and not query_info["projection"]:
        ...
        if isinstance(query_info, dict) and query_info["is_slow"] and query_info["query"]:
        ...
        if isinstance(query_info, dict) and query_info["operation"] == "find" and "sort" in query_info:
    ...
    def get_slow_queries(self) -> List[Dict[str, Any]]:
        ...
        """
        return [q for q in self.query_history if isinstance(q, dict) and q["is_slow"]]
...
async def run_optimization_loop(optimizer: ResourceOptimizer, interval: int = 1800):
    ...
    try:
        ...
        while True:
            ...
            if isinstance(check_result, dict) and check_result["needs_optimization"]:
