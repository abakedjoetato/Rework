...
async def initialize_db() -> AsyncIOMotorDatabase:
    ...
    except (pymongo.errorss.ServerSelectionTimeoutError, 
            pymongo.errorss.ConnectionFailure) as e:
        logger.critical(f"Failed to connect to MongoDB: {e}f")
        raise ConnectionError(f"Failed to connect to MongoDB: {e}f")
    except Exception as e:
        logger.critical(f"Unexpected error connecting to MongoDB: {e}f")
        raise ConnectionError(f"Unexpected error connecting to MongoDB: {e}f")
