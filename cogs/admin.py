...
class Admin(commands.Cog):
    ...
    async def setrole(self, ctx, role: discord.Role):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Cannot wipe premium server data without Force=True: server_id={server_id}")
    ...
    async def premium(self, ctx, guild_id: str, tier: int):
        ...
        try:
            ...
            try:
                ...
                async with asyncio.timeout(5.0):  # 5 second timeout
                    ...
                    if target_guild is None:
                        ...
                        logger.info(f"Found existing guild: {guild_id} - Current premium tier: {target_guild.get('premium_tier')}")
            ...
            if premium_already_set is None:
                logger.info(f"Setting premium tier for existing guild {guild_id} from {target_guild.get('premium_tier')} to {tier}")
                result = await target_guild.set_premium_tier(self.bot.db, tier)
                logger.info(f"Premium tier update result: {result} - Guild {guild_id} new tier: {target_guild.get('premium_tier')}")
            else:
    ...
    async def sethomeguild(self, ctx):
        ...
        try:
            ...
            # Check if user is not None is not None is the bot owner
                    ...
                    with open(".env", "w") as f:
                        ...
                        for line in lines:
                            if line.startswith("HOME_GUILD_ID="):
                                f.write(f"HOME_GUILD_ID={ctx.guild.id}f\n")
                                env_updated = True
                            else:
                                f.write(line)
                        
                        # Add the line if it doesn't exist
                        if env_updated is None:
                            f.write(f"HOME_GUILD_ID={ctx.guild.id}f\n")
